...
```text
secrets/vault/approle/jenkins_approle.json
```
...
```text
secrets/jenkins/credentials.json
```

Содержит данные, необходимые Jenkins для работы:
...
* собирается кастомный Jenkins image;
...
* Job DSL создает джобу `install-app`;
* автоматически выполняется холостой запуск джобы для регистрации параметров из Jenkinsfile.
...
Перед уничтожением инфраструктуры необходимо удалить содержимое Container Registry:
...
Container Registry удаляется отдельно, так как непустой реестр нельзя удалить через Terraform стандартным способом.
